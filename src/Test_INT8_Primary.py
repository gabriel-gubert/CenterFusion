...
import torch
from torch.ao.quantization import prepare, convert, get_default_qconfig, QuantWrapper, fuse_modules
import copy
...
def to_int8(detector, calib_data):
  def wrapper_forward(f):
    def forward(x, *args, **kwargs):
      y = f(x.to('cpu'), *args, **kwargs)
  ...
  for head in detector.model.heads:
    if head in detector.model.secondary_heads:
      continue

    detector.model.__getattr__(head).cpu()
    detector.model.__getattr__(head).qconfig = get_default_qconfig()
    detector.model.__setattr__(head, QuantWrapper(detector.model.__getattr__(head)))

    prepare(detector.model.__getattr__(head), inplace = True)

    detector.model.__getattr__(head).forward = wrapper_forward(detector.model.__getattr__(head).forward)
  ...
  for head in detector.model.heads:
    ...
    convert(detector.model.__getattr__(head), inplace = True)
...
def prefetch_test(opt):
  ...
  total_samples = len(calib_data_loader)
  subset_size = total_samples // 20

  np.random.seed(42) 

  random_idx = np.random.choice(total_samples, size=subset_size, replace=False).tolist()
  random_idx_set = set(random_idx) 

  calib_data = []

  for ind, (img_id, pre_processed_images) in enumerate(calib_data_loader):
      if ind in random_idx_set:
          calib_data.append(pre_processed_images)

  to_int8(detector, calib_data)
